...
struct Monkey {
    ...
    pair<ULL, int> turn_i(int i) {
        pair<ULL, int> item_move;
        perform_operation(i);
        this->items[i] %= level_manager;
    ...
    vector<pair<ULL, int>> turn() {
        this->inspections += items.size();
        vector<pair<ULL, int>> item_moves;
...
int main() {
    ...
    for(int i = 0; i < ROUNDS; ++i) {
        for(auto &monkey : monkeys) {
            vector<pair<ULL, int>> item_moves = monkey.turn();
